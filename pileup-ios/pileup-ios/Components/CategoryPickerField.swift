...
/// Clean item model for Category selection
struct CategoryPickerItem: Identifiable, Equatable {
    let id: Int?
    let name: String
    let color: Color
...
/// Unified, clean Category Picker Field that opens a native Bottom Sheet.
/// Matches the card height, badge style (38x38) and typography of the Accounts selector.
struct CategoryPickerField: View {
    var title: String? = nil
    var placeholder: String = "Seleziona categoria"
    let items: [CategoryPickerItem]
    ...
    private var selectedItem: CategoryPickerItem? {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isSheetPresented) {
            CategorySelectionSheet(
                title: showAllOption ? "Filtra Categoria" : "Seleziona Categoria",
                items: items,
...
/// Spacious Bottom Sheet for selecting a category with live search and clean rows.
struct CategorySelectionSheet: View {
    ...
    private var filteredItems: [CategoryPickerItem] {
        ...
        NavigationStack {
            VStack(spacing: 12) {
                ...
                HStack(spacing: 10) {
                    ...
                    TextField("Cerca categoria...", text: $searchQuery)
                        .font(.system(size: 15))
                        .foregroundColor(AppTheme.Colors.dynamicText)
                ...
                ScrollView(.vertical, showsIndicators: true) {
                    LazyVStack(spacing: 8) {
                        ...
                        if filteredItems.isEmpty {
                            ...
                            ForEach(filteredItems) { cat in
                                let isSelected = selectedId != nil && selectedId == cat.id
                                Button(action: {
                                    HapticHelper.selection()
                                    selectedId = cat.id
                                    onSelect?(cat.id)
                                    ...
                                    .background(
                                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                                            .fill(isSelected ? cat.color.opacity(0.08) : AppTheme.Colors.dynamicCardBackground)
                                    )
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                                            .stroke(isSelected ? cat.color : AppTheme.Colors.dynamicBorder.opacity(0.6), lineWidth: isSelected ? 1.5 : 1)
                                    )
                                }
                                .buttonStyle(.plain)
                            }
